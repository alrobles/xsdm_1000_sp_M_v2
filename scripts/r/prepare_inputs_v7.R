#!/usr/bin/env Rscript
# prepare_inputs_v7.R — Build v7 inputs for one species:
...
#   5. Sample 4 x n_pres pseudo-absences inside the buffered M (on land).
...
output_dir     <- parse_arg("--output_dir", "/home/a474r867/work/xsdm_1000_sp/outputs_M")
years_str      <- parse_arg("--years", paste(1980:2020, collapse = ","))
pa_factor      <- as.integer(parse_arg("--pa_factor", "1"))
seed           <- as.integer(parse_arg("--seed", "42"))
buffer_m_arg   <- parse_arg("--buffer_m", "median")
pa_in_buffer   <- tolower(parse_arg("--pa_in_buffer", "false")) %in% c("true", "1", "yes", "t")
...
# If pseudo-absences should be sampled in the buffer ring (M_buffer \ M)
pa_mask <- M_buffer
if (pa_in_buffer) {
...
# ─────────────────────────────────────────────────────────────────────────────
# 5. Sample pseudo-absences inside the chosen mask (buffer or buffered M), land only
# ─────────────────────────────────────────────────────────────────────────────
set.seed(seed + n_pres)
n_pa <- pa_factor * n_pres
...
pa_vect <- tryCatch(
  terra::spatSample(r_mask, size = n_pa, as.points = TRUE,
                    values = FALSE, na.rm = TRUE),
  error = function(e) NULL
)

if (is.null(pa_vect) || nrow(pa_vect) == 0) {
  stop("Could not sample any pseudo-absences inside the buffer area on land.", call. = FALSE)
}
...
cat(sprintf("\nPrepared inputs for %s in %s\n", species_name, sp_dir))
cat(sprintf("  presences: %d | pseudo-absences: %d\n", n_pres, nrow(pa)))
