...
source(file.path(script_dir, "compute_tss.R"), local = FALSE)
...
bbox_10 <- if (nchar(Sys.getenv("FIXED_BBOX",""))>0) { b <- as.numeric(strsplit(Sys.getenv("FIXED_BBOX"),",")[[1]]); make_bbox_polygon(data.frame(lon=b[1:2],lat=b[3:4]), buffer_km=0) } else { make_bbox_polygon(occ_vect, buffer_km = 10) }
dir.create(dirname(shapefile_out), recursive = TRUE, showWarnings = FALSE)
terra::writeVector(bbox_10, shapefile_out, overwrite = TRUE)
...
hab_bin_png <- file.path(dirname(output_png), "habitat_suitability_binary_v6.png")
hab_bin_tif <- file.path(dirname(output_png), "habitat_suitability_binary_v6.tif")
...
terra::plot(hab, main = "Habitat suitability")
terra::lines(bbox_10, col = "black", lwd = 2)
...
terra::plot(hab_bin, col = c("grey80", "darkgreen"), main = sprintf("Binary range (TSS threshold = %.3f)", thr))
terra::lines(bbox_10, col = "black", lwd = 2)
terra::points(pres_vect, col = "red", pch = 16, cex = 0.65)
terra::points(abs_vect, col = "grey40", pch = 1, cex = 0.5)
grDevices::dev.off()
...
tss_out <- file.path(species_dir, "tss_results_v6.rds")
compute_tss(
  model_rds    = model_rds,
  occ_csv      = occ_csv,
  bioclim_dir  = bioclim_dir,
...
message("Buffered bounding box shapefile written to: ", shapefile_out)
