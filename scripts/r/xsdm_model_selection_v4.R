#!/usr/bin/env Rscript
# xsdm_model_selection_v4.R
# ────────────────────────────────────────────────────────────────────────────
# Pipeline v4: 19 bioclim vars, 2-var models, 1 job per model at L1
#
# Stages:
...
cat(sprintf("=== xSDM v4 | %s | stage=%s", species_name, stage),
...
# ═══════════════════════════════════════════════════════════════════════════
# STAGE: L1_model — fit ONE 2-var model
# ═══════════════════════════════════════════════════════════════════════════
...
if (stage == "L1_model") {
  ...
  if (!is.null(fit)) {
    ...
    saveRDS(fit, file.path(l1_model_dir, sprintf("model_%02d.rds", model_index)))
    # Write per-model marker
    writeLines(as.character(Sys.time()),
               file.path(l1_model_dir, sprintf(".model_%02d_done", model_index)))
...
if (stage == "L2") {
  rpt(paste0("# xSDM v4 Report: *", species_name, "*"))
  rpt("")
  ...
  saveRDS(list(species = species_name, stage = "L2", L1 = L1, L2 = L2,
               eligible_L1 = eligible, tau = tau, best_pBIC_L1 = best_pBIC_L1),
          file.path(p1_dir, "L2_boundary.rds"))

  writeLines(as.character(Sys.time()), file.path(p1_dir, ".L2_done"))
...
if (stage == "L4") {
  ...
  rpt(sprintf("**Variables:** %s (2-var)", paste(L3_data[[M_Omega]]$vars, collapse=", ")))
  rpt("")
  ...
  saveRDS(list(species = species_name, stage = "L4_final", M_Omega = M_Omega,
               Omega = Omega, L1 = L1, L2 = L2, L3_order = l3_data$L3_order,
               L4 = L4, all_models = all_models),
          file.path(p1_dir, "phase1_results.rds"))

  saveRDS(list(M_Omega = M_Omega, Omega = Omega, L4 = L4),
          file.path(p1_dir, "L4_final.rds"))

  writeLines(as.character(Sys.time()), file.path(p1_dir, ".L4_done"))
  flush_report(file.path(p1_dir, "report.md"))
